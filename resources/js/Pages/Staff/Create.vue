...
onMounted(() => {
    const flatpickrConfig = {
        ...
        onChange: (selectedDates, dateStr, instance) => {
            if (instance.element === dobInput.value) form.DOB = dateStr;
            if (instance.element === joiningDateInput.value)
                form.StartDate = dateStr;
        },
    };
    flatpickr(dobInput.value, flatpickrConfig);
    flatpickr(joiningDateInput.value, flatpickrConfig);
});
...
const submit = () => {
    form.post(route("staff.store"), {
        ...
        onSuccess: () => {
            Swal.fire({
                title: "ជោគជ័យ!",
                text: "ទិន្នន័យមន្ត្រីត្រូវបានរក្សាទុកដោយជោគជ័យ",
                icon: "success",
                customClass: {
                    title: "font-siemreap",
...
const filteredPlans = computed(() => {
    if (form.UnitID == 1) {
        return props.plans.filter((plan) => plan.unit_id == 1);
    }
    return [];
});
const filteredOffices = computed(() => {
    if (!form.PlanID) return [];
    return props.offices.filter((office) => office.plan_id == form.PlanID);
});
...
const filteredSections = computed(() => {
    ...
    if (form.UnitID == 1) {
        ...
        return allSections.filter((s) => s.office_id == form.OfficeID);
    }
    return allSections.filter((s) => s.unit_id == form.UnitID);
});

const filteredPosts = computed(() => {
    if (!form.UnitID) return [];

    return props.posts.filter((post) => post.unit_id == form.UnitID);
});
...
<template>
    <Head title="បញ្ចូលឈ្មោះមន្ត្រី | ស្នងការដ្ឋាននគរបាលខេត្តប៉ៃលិន" />
            ...
            >
                ...
                <div class="bg-white p-8 rounded-2xl shadow-sm w-full md:w-3/4">
                    ...
                    <div class="grid grid-cols-1 gap-4 mx-20 font-siemreap">
                        ...
                        <div class="flex flex-col md:flex-row md:items-center">
                            <label class="w-72 text-gray-700">អត្តលេខ*:</label>
                            <div class="w-full flex flex-col">
                                <input
                        ...
                        <div class="flex flex-col md:flex-row md:items-center">
                            <label class="w-72 text-gray-700">អង្គភាព*:</label>
                            <div class="flex flex-col w-full">
                                <select
                                    v-model="form.UnitID"
                        ...
                        <div class="flex flex-col md:flex-row md:items-center">
                            ...
                            >
                                <option value="">ជ្រើសរើសផែន</option>
                                <option
                        ...
                        <div class="flex flex-col md:flex-row md:items-center">
                            ...
                            <select
                                ...
                                :disabled="!form.PlanID || !!form.PostID"
                                class="mt-1 block w-full border border-gray-300 leading-7 outline-none transition-all rounded-md p-2 text-sm"
                                :class="[
                                    !form.PlanID || !!form.PostID
                                        ? 'bg-gray-100 cursor-not-allowed text-gray-400'
                                        : 'bg-white cursor-pointer focus:border-blue-500 focus:ring-2 focus:ring-blue-200',
                                    form.OfficeID === ''
                                        ? 'text-gray-500'
                        ...
                        <div class="flex flex-col md:flex-row md:items-center">
                            <label class="w-72 text-gray-700">ផ្នែក*:</label>
                            <select
                        ...
                        <div class="flex flex-col md:flex-row md:items-center">
                            <label class="w-72 text-gray-700">ប៉ុស្តិ៍*:</label>
                            <select
                                v-model="form.PostID"
                                :disabled="
                                    !form.UnitID ||
                            ...
                            >
                                <option value="">ជ្រើសរើសប៉ុស្តិ៍</option>
                                <option
                        ...
                        <div class="flex flex-col md:flex-row md:items-center">
                            ...
                            <div class="w-full">
                                <input
                                    ...
                                    :class="[
                                        'mt-1 block w-full border border-gray-300 leading-7 outline-none transition-all focus:border-blue-500 focus:ring-2 focus:ring-blue-200 rounded-md p-2 text-sm',
                                    ...
                                    >
                                        <div class="flex items-center gap-3">
                                            ...
                                            <button
                                                ...
                                                class="text-blue-600 hover:text-blue-800 underline font-siemreap text-sm decoration-1 underline-offset-4"
                                            >
                                                {{ file.name }}
                                            </button>
                                        </div>
